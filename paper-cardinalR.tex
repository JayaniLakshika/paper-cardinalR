...
The S-curve structure represents a smooth, non-linear manifold in 3D space. Using \texttt{gen\_scurve(n,\ p)} (Figure \ref{fig:scurve-proj} a), the geometry is defined by a latent parameter \(\theta \sim U(-3\pi/2, 3\pi/2)\), which controls the curvature of the manifold. The first three coordinates form the S-shape:
