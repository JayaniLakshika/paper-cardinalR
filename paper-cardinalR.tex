...
\begin{figure}[!ht]
\includegraphics[width=1\linewidth,alt={The figure shows a 2-D scatter plot of projected branch structures in a high-dimensional space. The horizontal axis represents one coordinate (first index of the active pair) and the vertical axis represents the other coordinate (second index of the active pair), each spanning a moderate, symmetric range around zero (for example, roughly −2 to 2). Points are grouped visually into several distinct linear or gently curving branches, each corresponding to a different active 2-D subspace; these branches appear as separate streaks or arcs radiating through the plane. Colour (or another visual attribute such as line/point grouping) is used to distinguish branches that occupy different subspaces, and some branches may overlap or run close together where subspaces are shared. The main pattern is a set of well-separated, structured branches with little noise, illustrating how high-dimensional linear or curvy branches project into 2-D coordinate pairs.}]{paper-cardinalR_files/figure-latex/branch-proj-1} \caption{Three $2\text{-}D$ projections from the $4\text{-}D$ \texttt{orgcurvybranches} data. Each shows a different projection, illustrating how the linear branches appear from multiple viewing angles. These views highlight the dataset’s underlying branching structure and demonstrate how projections reveal patterns that are otherwise hidden in higher dimensions.}\label{fig:branch-proj}
\end{figure}
...
\begin{figure}[!ht]
\includegraphics[width=1\linewidth,alt={ A 2D scatterplot shows the projection of a high-dimensional cone-shaped point cloud. The horizontal and vertical axes are unlabeled numeric coordinates, each spanning a roughly symmetric range around zero (for example, from about -1 to 1). Hundreds of points form a dense cluster near one end of the plot, corresponding to the cone’s apex, with density gradually decreasing as points extend outward in a flared, cone-like or triangular region. There are no colours, shapes, or facets used; all points share the same appearance, and the key visible pattern is the concentrated tip transitioning smoothly into a wider spread of points, illustrating a cone structure in projection.}]{paper-cardinalR_files/figure-latex/cone-proj-1} \caption{Three $2\text{-}D$ projections from the $3\text{-}D$ \texttt{cone} data. Points are concentrated near the tip along the height dimension, while the radius of the hyperspherical cross-section decreases linearly toward the apex. These projections show how the conical geometry is preserved.}\label{fig:cone-proj}
\end{figure}
