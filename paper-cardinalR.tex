...
\begin{figure}[!ht]
\includegraphics[width=1\linewidth,alt={Three $2\text{-}D$ projections of the $4\text{-}D$ orglinearbranches dataset, showing different angles of the linear branches to illustrate the branching structure.}]{paper-cardinalR_files/figure-latex/branch-proj-1} \caption{Three $2\text{-}D$ projections from the $4\text{-}D$ data generated using \texttt{gen\_orgcurvybranches}. Each shows a different projection, illustrating how the linear branches appear from multiple viewing angles. These views highlight the dataset’s underlying branching structure and demonstrate how projections reveal patterns that are otherwise hidden in higher dimensions.}\label{fig:branch-proj}
\end{figure}
