...
The \texttt{gen\_gridedsphere(n,\ p)} function generates a \(p\)-dimensional dataset of approximately \(n\) points evenly distributed on the surface of a \((p!-!1)\)-sphere (the unit sphere embedded in \(\mathbb{R}^p\)).
...
The function \texttt{gen\_trefoil4d()} generates a \(4\text{-}D\) trefoil knot using two angular parameters, \(\theta\) and \(\phi\). The knot is constructed on the \(3\)-sphere (\(S^3 \subset \mathbb{R}^4\)), with a band of thickness around the knot path, controlled by \texttt{steps}.
