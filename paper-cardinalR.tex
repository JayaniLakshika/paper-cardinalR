...
The \texttt{gen\_expbranches(n,\ p,\ k)} function generates a dataset of \(n\) points forming \(k\) exponential branches in 2D, with optional noise dimensions to embed the structure in a \$p\$-dimensional space. These branches grow in opposite directions, producing a radiating curvilinear structure from a central region.
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-expbranches-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `expbranches` data.}\label{fig:fig-expbranches-proj}
\end{figure}

The \texttt{gen\_linearbranches(n,\ p,\ k)} function generates a dataset of \(n\) points forming \(k\) approximately linear branches in a \(p\)-dimensional space. The core structure lies in the first two dimensions and additional dimensions carry Gaussian noise.
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-linearbranches-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `linearbranches` data.}\label{fig:fig-linearbranches-proj}
\end{figure}

The \texttt{gen\_curvybranches(n,\ p,\ k)} function generates a dataset of \(n\) points forming \(k\) curvilinear branches embedded in a \(p\)-dimensional space. The underlying geometry lies in the first two dimensions, while the remaining \(p - 2\) dimensions contain Gaussian noise.
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-curvybranches-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `curvybranches` data.}\label{fig:fig-curvybranches-proj}
\end{figure}

The \texttt{gen\_orglinearbranches(n,\ p,\ k)} function generates a dataset of \(n\) points forming \(k\) approximately linear branches embedded in a \(p\)-dimensional space. Each branch lies primarily within a distinct \(2\text{-}D\) subspace, while the remaining \(p - 2\) dimensions contain Gaussian noise.
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-orglinearbranches-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `orglinearbranches` data.}\label{fig:fig-orglinearbranches-proj}
\end{figure}
...
Each branch contains \(n\_i\) points such that \(\sum\_{i=1}^k n\_i = n\), where the vector \((n\_1, \dots, n\_k)\) is randomly drawn using the helper function \texttt{gen\_nsum()} to partition \(n\).
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-orgcurvybranches-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `orgcurvybranches` data.}\label{fig:fig-orgcurvybranches-proj}
\end{figure}
...
This function generates \texttt{n} points in a \texttt{p}-dimensional space, where the last dimension (\texttt{X\_p}) represents height along the cone's axis, and the remaining dimensions define a shrinking hyperspherical cross-section as one moves toward the tip.
...
Each variable \(X\_j\) represents integer grid coordinates (e.g., 1, 2, 3, \ldots) in dimension \(j\):
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-gridcube-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `gridcube` data.}\label{fig:fig-gridcube-proj}
\end{figure}

The \texttt{gen\_unifcube(n,\ p)} function generates \(n\) points uniformly distributed within a \(3\text{-}D\) cube centered at the origin, optionally embedded in a higher-dimensional space with additional noise dimensions.
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-unifcube-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `unifcube` data.}\label{fig:fig-unifcube-proj}
\end{figure}

The \texttt{gen\_cubehole(n,\ p)} function generates a dataset of \(n\) points sampled uniformly inside a cube, then removes points that fall within a central spherical hole, resulting in a hollow cube structure in \(p\)-dimensional space.
...
For \(p > 3\), Gaussian noise is added to the remaining dimensions (via \texttt{gen\_unifcube()}):
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-cubehole-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `cubehole` data.}\label{fig:fig-cubehole-proj}
\end{figure}

\subsubsection{Gaussian}\label{gaussian}

The \texttt{gen\_gaussian(n,\ p,\ s)} function generates a multivariate Gaussian cloud in \(p\)-dimensional space, centered at the origin with user-defined covariance structure.
...
The \texttt{gen\_longlinear(n,\ p)} function simulates a high-dimensional linear structure with a dominant linear trend and small additive noise.
...
The \texttt{gen\_mobius(n,\ p)} function generates a dataset of \(n\) points that form a Mobius strip embedded in the first three dimensions of a \(p\)-dimensional space. This classical non-orientable surface loops back on itself with a half-twist.

The Mobius strip is generated using \texttt{geozoo::mobius(n,\ p\ =\ 3)\$points}, which samples \(n\) points from a parametrization of the surface defined by angle \(\theta\) and strip width \(w\). The cartesian coordinates are;
...
The \texttt{gen\_quadratic(n,\ p,\ range)} function generates a dataset of \(n\) points forming a quadratic curve in the first two dimensions of a \(p\)-dimensional space. This \(2\text{-}D\) parabolic structure is embedded within a higher-dimensional space with additive noise in the remaining dimensions.
...
For \(p > 2\), the dataset includes Gaussian noise in the remaining dimensions via \texttt{gen\_noisedims()}:
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-quadratic-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `quadratic` data.}\label{fig:fig-quadratic-proj}
\end{figure}

The \texttt{gen\_cubic(n,\ p,\ range)} function generates a dataset of \(n\) points forming a cubic curve in the first two dimensions of a \(p\)-dimensional space. This function creates a more complex curvilinear structure than a simple parabola.
...
When \(p > 2\), the remaining dimensions consist of independent Gaussian noise features generated via \texttt{gen\_noisedims()}:
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-cubic-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `cubic` data.}\label{fig:fig-cubic-proj}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-pyrrect-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `pyrrect` data.}\label{fig:fig-pyrrect-proj}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-pyrtri-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `pyrtri` data.}\label{fig:fig-pyrtri-proj}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-pyrstar-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `pyrstar` data.}\label{fig:fig-pyrstar-proj}
\end{figure}

The \texttt{gen\_pyrholes(n,\ p)} function generates \(n\) points embedded in a \(p\)-dimensional simplex using a chaotic attractor-like midpoint algorithm. The result is a fractal-like structure that reveals holes or gaps in the data cloud, forming a ``Sierpinski-like pyramid'' in high dimensions.
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-pyrholes-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `pyrholes` data.}\label{fig:fig-pyrholes-proj}
\end{figure}
...
To simulate an S-curve structure in a higher-dimensional space, we define the function \texttt{gen\_scurve(n,\ p)}, which generates \texttt{n} observations in \texttt{p} dimensions.
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-scurve-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `scurve` data.}\label{fig:fig-scurve-proj}
\end{figure}

To simulate a variant of the S-curve structure with a removed region (``hole''), we define the function \texttt{gen\_scurvehole(n,\ p)}. This function produces \texttt{n} observations in a \(p\)-dimensional space where the first three dimensions describe the S-curve manifold, and remaining dimensions add low-variance Gaussian noise. A subset of observations near a designated anchor point is excluded to introduce a hole in the manifold.
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-scurvehole-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `scurvehole` data.}\label{fig:fig-scurvehole-proj}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-circle-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `circle` data.}\label{fig:fig-circle-proj}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-curvycycle-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `curvycycle` data.}\label{fig:fig-curvycycle-proj}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-unifsphere-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `unifsphere` data.}\label{fig:fig-unifsphere-proj}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-gridedsphere-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `gridedsphere` data.}\label{fig:fig-gridedsphere-proj}
\end{figure}

The \texttt{gen\_clusteredspheres(n,\ k,\ p,\ r,\ loc)} function generates a synthetic dataset of \(n_1 + k \cdot n_2\) observations in \(p\)-dimensional space, consisting of one large sphere of radius \(r_1\) and \(k\) smaller spheres of radius \(r_2\), each centered at a different random location.

A large uniform sphere centered at the origin is created by sampling \(n_1\) points uniformly on the surface of a \(p\)-dimensional sphere with a radius of \(r_1\). The sampling is executed using the function \texttt{gen\_unifsphere(n\_1,\ p,\ r\_1)}, which generates the desired points in the specified dimensional space.
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-clusteredspheres-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `clusteredspheres` data.}\label{fig:fig-clusteredspheres-proj}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-hemisphere-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `hemisphere` data.}\label{fig:fig-hemisphere-proj}
\end{figure}

\subsubsection{Swiss Roll}\label{swiss-roll}

To further generalize the Swiss roll structure and introduce realistic noise, we define a function \texttt{gen\_swissroll(n,\ p,\ w)}, where \texttt{n} is the number of points, \texttt{p} is the total number of dimensions, and \texttt{w} is the vertical range in the third dimension.
...
For \(p > 2\), additional dimensions \(X_3\) to \(X_p\) are generated using \texttt{gen\_wavydims1()}:
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-crescent-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `crescent` data.}\label{fig:fig-crescent-proj}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-curvycylinder-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `curvycylinder` data.}\label{fig:fig-curvycylinder-proj}
\end{figure}
...
If \(p > 4\), the function appends structured, non-linear noise via \texttt{gen\_wavydims2()}. These dimensions are functions of the first coordinate \(X_1\), introducing dependencies that preserve some geometric coherence. Each added dimension follows the form:
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-sphericalspiral-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `sphericalspiral` data.}\label{fig:fig-sphericalspiral-proj}
\end{figure}

The \texttt{gen\_helicalspiral(n,\ p)} function generates a dataset of \(n\) observations forming a helical spiral, embedded in the first four dimensions of a \(p\)-dimensional space. Additional dimensions (if \(p > 4\)) are filled with structured noise to mimic high-dimensional complexity.
...
When \(p > 4\), the remaining \(p - 4\) dimensions are populated using \texttt{gen\_noisedims()}. These are independent Gaussian noise dimensions:
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-helicalspiral-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `helicalspiral` data.}\label{fig:fig-helicalspiral-proj}
\end{figure}
...
For \(p > 4\), the dataset includes isotropic Gaussian noise in the remaining dimensions via \texttt{gen\_noisedims()}:
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-conicspiral-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `conicspiral` data.}\label{fig:fig-conicspiral-proj}
\end{figure}
...
\item
  \(X_2 = \frac{hc}{X_1} + \text{non\_fac} \cdot \sin(X_1)\): non-linear combination of \textbf{hyperbolic and sinusoidal transformations}, creating sharp curvature and oscillation.
\item
...
If \(p > 4\), the remaining dimensions are populated with Gaussian noise using \texttt{gen\_noisedims()}:
...
\begin{figure}
\includegraphics[width=1\linewidth]{paper-cardinalR_files/figure-latex/fig-nonlinear-proj-1} \caption{Three $2\text{-}D$ projections from $4\text{-}D$, for the `nonlinear` data.}\label{fig:fig-nonlinear-proj}
\end{figure}
