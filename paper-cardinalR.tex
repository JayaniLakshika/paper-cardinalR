...
The \texttt{gen\_gridcube(n,\ p)} constructs a regular grid using approximately \(n\) points by evenly partitioning each of the \(p\) dimensions (Figure \ref{fig:cube-proj} a). Specifically, the number of grid points along each axis is determined by \texttt{gen\_nproduct(n,\ p)}, which calculates the nearest balanced integer factors whose product is close to \(n\). The full grid is generated via \texttt{expand\_grid()}, giving Cartesian combinations of these evenly spaced positions.
...
The \texttt{gen\_unifcube(n,\ p)} function generates \(n\) points uniformly distributed within a \(3\text{-}D\) cube centered at the origin, optionally embedded in \(p\text{-}D\) with additional noise dimensions (Figure \ref{fig:cube-proj} b).
...
The \texttt{gen\_cubehole(n,\ p)} function generates a dataset of \(n\) points sampled uniformly inside a cube, then removes points that fall within a central spherical hole, resulting in a hollow cube structure in \(p\text{-}D\) (Figure \ref{fig:cube-proj} c).
...
The \texttt{gen\_gaussian(n,\ p,\ s)} function generates a multivariate Gaussian cloud in \(p\text{-}D\), centered at the origin with user-defined covariance structure (Figure \ref{fig:gau-proj}). Each point is independently drawn using the multivariate normal distribution with \(X_i \sim N_p(\boldsymbol{0}, \Sigma)\), where \(\Sigma\) is a user-defined \(p \times p\) positive-definite matrix.
