...
Several R packages address this need from different perspectives. For example, \texttt{geozoo} (\citet{barret2016}) provides a wide range of geometric objects, primarily in low dimensions; \texttt{snedata} (\citet{james2025}) focuses on datasets for dimensionality reduction; \texttt{mlbench} (\citet{friedrich2024}) supplies benchmark datasets for classification and regression; and \texttt{splatter} (\citet{luke2017}) specializes in simulating single-cell omics data. While each serves its own domain well, none are designed for flexible, high-dimensional data generation with customizable geometry, noise, and clustering.
