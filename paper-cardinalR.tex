...
The \texttt{gen\_curvybranches(n,\ p,\ k)} function generates a dataset of \(n\) points forming \(k\) curvilinear branches embedded in \(p\text{-}D\). The underlying geometry lies in the first two dimensions, while the remaining \((p-2)\text{-}D\) dimensions contain Gaussian noise.
