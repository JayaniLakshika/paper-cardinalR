...
High-dimensional generalizations are provided by \texttt{gen\_orglinearbranches(n,\ p,\ k)} and \texttt{gen\_orgcurvybranches(n,\ p,\ k)} (Figure \ref{fig:branch-proj} d-e). Each branch is embedded in a unique or repeated \(2\text{-}D\) subspace of the \(p\text{-}D\) space. Linear branches follow
