...
The \texttt{gen\_expbranches(n,\ p,\ k)} function generates a dataset of \(n\) points forming \(k\) exponential branches in \(2\text{-}D\), with optional noise dimensions to embed the structure in a \(p\)-dimensional space. These branches grow in opposite directions, producing a radiating curvilinear structure from a central region.
...
Each branch contains \(n_i\) points such that \(\sum_{i=1}^k n_i = n\), where the vector \((n_1, \dots, n_k)\) is randomly drawn using the helper function \texttt{gen\_nsum()} to partition \(n\).
...
Each variable \(X_j\) represents integer grid coordinates (e.g., 1, 2, 3, \ldots) in dimension \(j\):
...
Let \(x_1, x_2, \dots, x_p\) denote the coordinates of the generated points. The final coordinate \$x\_p\$ encodes the height \(z\) of each point and is drawn from an exponential distribution capped at \(h\):
...
\(X_{2} = \frac{hc}{X_{1}} + \text{nonfac} \cdot \sin(X_{1})\): non-linear combination of \textbf{hyperbolic and sinusoidal transformations}, creating sharp curvature and oscillation.
