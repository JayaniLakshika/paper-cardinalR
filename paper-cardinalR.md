...
The `gen_gridedsphere(n, p)` function generates a $p$-dimensional dataset of approximately $n$ points evenly distributed on the surface of a $(p!-!1)$-sphere (the unit sphere embedded in $\mathbb{R}^p$).

The construction is based on a regular grid in spherical coordinates with $(p-1)$ angular variables. Specifically:

* For dimensions $j = 1, \dots, p-2$, the polar angles are defined on $[0, \pi]$.
* For the last angle ($j = p-1$), the azimuthal angle is defined on $[0, 2\pi]$.
