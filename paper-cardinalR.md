...
$$
X_j \sim \mathcal{N}(0, 0.1^2), \quad j = 3, \dots, p.
$$
...
Each branch is a segment of a line with added jitter to simulate measurement noise. The branches differ in direction and location. Branches 1 and 2 are initialized with fixed slopes and intercepts. The Branch 1 is generated from $X_1 \sim \mathcal{U}(-2, 8)\$, \$X_2 = 0.5 \cdot X_1 + \epsilon$. The Branch 2 is generated from $X_1 \sim \mathcal{U}(-6, 2)\$, \$X_2 = -0.5 \cdot X_1 + \epsilon$. $\epsilon \sim \mathcal{U}(0, 0.5)$. 
...
$X_2 = s_i \cdot (X_1 - x_{start}) + y_{start} + \epsilon$ where $s_i$ is a sampled slope from a filtered set and $\epsilon \sim \mathcal{U}(0, 0.2)$.
...
$$
X_j \sim \mathcal{N}(0, 0.05^2)\text{ for }j = 3, \dots, p.
$$
...
Branch 1 is generated from $X_1 \sim \mathcal{U}(0, 1)$, and $X_2 = 0.1 \cdot X_1 + 1 \cdot X_1^2 + \epsilon$, where $\epsilon \sim \mathcal{U}(0, 0.05)$. This produces a gently upward-curving parabola in the right half-plane.

Branch 2 is generated from $X_{1} \sim \mathcal{U}(-1, 0)$, and $X_2 = 0.1 \cdot X_{1} - 2 \cdot X_{1}^2 + \epsilon$, where $\epsilon \sim \mathcal{U}(0, 0.05)$. This creates a steeper, leftward-facing curve in the left half-plane.
...
$$
X_j \sim \mathcal{N}(0, 0.05^2), \quad \text{for } j = 3, \dots, p.
$$
...
Points are sampled independently from $X_1, X_2, X_3 \sim \mathcal{U}(-0.5, 0.5)$ of a cube of side length 1. This results in a uniform cloud of points filling the unit cube in $3\text{-}D$ space.
...
The initial points are drawn from a uniform cube centered at the origin: $X_1, X_2, X_3 \sim \mathcal{U}(-0.5, 0.5)$.
...
The curve is constructed by drawing uniformly spaced inputs and applying a second-degree polynomial transformation. Let $X_1 \sim \mathcal{U}(\text{range}[1], \text{range}[2])$ be the independent variable. A raw polynomial basis of degree 2 is used to form $X_2 = X_1 - X_1^2 + \varepsilon_2$, where $\varepsilon_2 \sim \mathcal{U}(0, 0.5)$.
...
The shape is generated using a third-degree raw polynomial basis expansion. Let $X_1 \sim \mathcal{U}(\text{range}[1], \text{range}[2])$ be the base input. A cubic transformation with vertical jitter is used to define $X_2 = X_1 + X_1^2 - X_1^3 + \varepsilon_2\$, where \$\varepsilon_2 \sim \mathcal{U}(0, 0.5)$.

When $p > 2$, the remaining dimensions consist of independent Gaussian noise features generated via `gen_noisedims()`:

$$
X_j \sim \mathcal{N}(0, 0.1^2)\$ for \$j = 3, \dots, p.
$$
