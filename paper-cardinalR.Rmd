...
```{r cone, eval=knitr::is_html_output(), fig.cap="Viewing the $4\\text{-}D$ output  of `cone` data. Points are concentrated near the tip along the height dimension, while the radius of the hyperspherical cross-section decreases linearly toward the apex.", fig.alt="The figure shows a tour of the 4‑D cone dataset. Points are displayed in a continuously changing 2‑D projection, where each axis represents a linear combination of the original four dimensions. Points are densely packed near one end, forming a narrow tip, and gradually spread out along the other direction, creating a wider base. All points share the same colour and marker. As the projection rotates, the cone-like structure shifts in orientation, maintaining roughly square aspect ratios throughout the animation and showing the distribution of points from multiple viewing angles.", fig.pos="!ht"}
...
```{r highd-data-clusters-algo-html, eval=knitr::is_html_output(), fig.cap="Views of the five-cluster synthetic dataset coloured by the $k$-means four- and five-cluster solutions. These show how the five true structures: helical spiral, hemisphere, uniform cube, blunted cone, and Gaussian-shaped cluster appear under $k$-means partitions. Both the $4$- and $5$-cluster solutions mix colours, revealing substantial misclassification and limited ability of $k$-means to recover the underlying geometric structure.", fig.alt="The figure contains two side-by-side tour views of the same synthetic high-dimensional dataset, each shown as a sequence of continuously changing 2-D projections. The left tour view is coloured by a four-cluster k-means solution, and the right tour view is coloured by a five-cluster k-means solution. In both views, each point represents a data observation, and colours indicate cluster membership. The horizontal and vertical axes correspond to linear projections of the original variables. Across the tours, points form rotating and deforming point clouds that intermittently reveal curved, elongated, compact, and diffuse structures. Colours are intermingled throughout most projections in both panels, with clusters frequently overlapping and splitting as the view changes. The two tours differ only in the number of colours used, with four colours on the left and five on the right, while the underlying point positions and projection motion remain the same.", fig.pos="!ht", layout = "l-body"}
