...
The S-curve structure (`r knitr::asis_output(if (knitr::is_html_output()) { "Figure \\@ref(fig:scurve)"} else { "Figure \\@ref(fig:scurve-proj)"})`) represents a smooth, non-linear manifold in 3D space. Using `gen_scurve(n, p)` (`r knitr::asis_output(if (knitr::is_html_output()) { "Figure \\@ref(fig:scurve) a"} else { "Figure \\@ref(fig:scurve-proj) a"})`), the geometry is defined by a latent parameter $\theta \sim U(-3\pi/2, 3\pi/2)$, which controls the curvature of the manifold. The first three coordinates form the S-shape:
